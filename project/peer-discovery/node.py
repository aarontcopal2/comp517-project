...
class Node(threading.Thread):
    ...
    def connect_with_node(self, host, port, reconnect=False):
        ...
        try:
            ...
            info = self.return_node_info()
            sock.send(info.encode('utf-8')) # Send my id to the connected node!
            connected_node_info = sock.recv(4096).decode('utf-8') # When a node is connected, it sends it id!
            connected_node_id   = connected_node_info.split()[0]
            connected_node_host = connected_node_info.split()[1].split(':')[0]
            connected_node_port = connected_node_info.split()[1].split(':')[1]
    ...
    def create_new_connection(self, connection, id, host, port, connected_node_host, connected_node_port):
        """When a new connection is made, with a node or a node is connecting with us, this method is used
           ...
           the node connection."""
        return NodeConnection(self, connection, id, host, port, connected_node_host, connected_node_port)
        ...
        while not self.terminate_flag.is_set():  # Check whether the thread needs to be closed
            try:
                ...
                if self.max_connections == 0 or len(self.nodes_inbound) < self.max_connections:
                    ...
                    # Basic information exchange (not secure) of the id's of the nodes!
                    connected_node_info = connection.recv(4096).decode('utf-8') # When a node is connected, it sends it id!
                    connected_node_id   = connected_node_info.split()[0]
                    connected_node_host = connected_node_info.split()[1].split(':')[0]
                    connected_node_port = connected_node_info.split()[1].split(':')[1]
            
                    connection.send(self.return_node_info().encode('utf-8')) # Send my id to the connected node!
    ...
    def return_node_info(self):
        return self.id + " " + self.host + ":" + str(self.port)
